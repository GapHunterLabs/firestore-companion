package dev.gaphunter.firestorecompanion.license
 ...
 * src/main/java/com/company/license/CheckLicense.java), same file
 * already verified and shipped in ansible-companion's own
 * `completion/CheckLicense.kt` -- copied verbatim here, only the
...
object CheckLicense {
    ...
    private const val TIMESTAMP_VALIDITY_PERIOD_MS = 1 * HOUR
    ...
    private fun showRegisterDialog(productCode: String, message: String?) {
        ...
        if (registerAction != null) {
            // Deliberately deprecated overload -- same choice already
            // verified against real IDE builds in ansible-companion's
            // copy of this file: the modern replacement
            // (ActionUtil.performAction(AnAction, AnActionEvent)) only
            // exists from IDE 244 onward, an unresolved invokestatic
            // risk against this plugin's sinceBuild=243. This overload
            // resolves everywhere and isn't @ApiStatus.OverrideOnly/
            // Internal/Experimental, so verifyPlugin stays clean.
